...
final_manifest <- final_manifest %>%
  ...
  mutate(
    ...
    # Initialize blank coordinator fields (Col 14 to 19 / N to S) [cite: 378]
...
writeData(wb, sheet_name_inst, "Operational guidelines and evaluation dictionary for clinical coordinators (v6 - collapsed flags)", startCol = 1, startRow = 2)
...
# Build mapping dictionary table (19 columns total with single accessibility_flags)
mapping_dict <- data.frame(
  ...
  Data_Source = c(
    ...
    "Model generated", "Model generated", "Coordinator log", "Coordinator log", "Coordinator log",
    "Coordinator log", "Coordinator log", "Coordinator log"
  )
)
...
eval_notes <- c(
  ...
  "• Core metrics: 'trial_arm' predicts 'cancel_outcome' (derived from 'final_attendance' == 'Cancelled').",
  "",
  ...
  "• Core metrics: 'tier3_phone_attempt' segments exposure tiers; 'dna_risk' is used as a log-offset."
)
...
# Add summary cards (KPI Block pointing to correct shifted columns for v6)
...
writeFormula(wb, sheet_name_rost, sprintf("COUNTIF(P%d:P%d, \"yes\")", start_row, end_row), startCol = 5, startRow = 4)
addStyle(wb, sheet_name_rost, style_kpi_value, rows = 4, cols = 5)

# Column headers (19 Columns total)
headers <- c(
  "appointment_id", "patient_id", "appointment_datetime", "clinic_code", "gp_practice", 
  "age", "sex", "ethnicity", "imd", "accessibility_flags", "dna_risk", "risk_label", "trial_arm", 
  "tier1_text_timestamp", "tier2_text_timestamp", "tier3_phone_attempt", "tier3_call_timestamp", 
...
addStyle(wb, sheet_name_rost, style_header, rows = 8, cols = 1:19)

# -------------------------------------------------------------------------
# Vectorized Pre-Population inside R (Loop-Free!)
# -------------------------------------------------------------------------
export_roster_data <- final_manifest %>% 
  ...
  mutate(
    # Set text reminders and call variables dynamically based on trial arms
...
# Write all 19 columns to Excel in a SINGLE high-speed operation
...
# -------------------------------------------------------------------------
addStyle(wb, sheet_name_rost, style_text_left, rows = start_row:end_row, cols = c(1, 2, 4, 5, 8, 10), gridExpand = TRUE)
...
# Decision Model outcomes formatting (Columns 11-13 / K-M)
addStyle(wb, sheet_name_rost, style_prob_pct, rows = start_row:end_row, cols = 11, gridExpand = TRUE) # DNA Risk Pct
addStyle(wb, sheet_name_rost, style_text_left, rows = start_row:end_row, cols = 12:13, gridExpand = TRUE) # Risk Label, Arm

# -------------------------------------------------------------------------
# Group-Based Vectorized Conditional Formatting
# -------------------------------------------------------------------------
if (length(intervention_rows) > 0) {
  # Active Intervention: Highlight all text and call columns 14 to 19 (N-S)
  addStyle(wb, sheet_name_rost, style_active_input, rows = intervention_rows, cols = 14:19, gridExpand = TRUE)
}

if (length(control_rows) > 0) {
  # Control group: Highlight text timestamps (14:15) in blue, phone cols (16:19) in gray
  addStyle(wb, sheet_name_rost, style_active_input, rows = control_rows, cols = 14:15, gridExpand = TRUE)
  addStyle(wb, sheet_name_rost, style_control_row, rows = control_rows, cols = 16:19, gridExpand = TRUE)
}

if (length(not_in_trial_rows) > 0) {
  # Not in Trial: Highlight all logging fields (14:19) in gray
  addStyle(wb, sheet_name_rost, style_control_row, rows = not_in_trial_rows, cols = 14:19, gridExpand = TRUE)
}

# Apply dynamic data validation dropdown lists for active outreach columns (Columns P/16 & R/18)
dataValidation(wb, sheet_name_rost, col = 16, rows = start_row:end_row, type = "list", value = '\"yes,no\"')
dataValidation(wb, sheet_name_rost, col = 18, rows = start_row:end_row, type = "list", value = '\"confirm,cancel,reschedule,no_response\"')
...
setColWidths(wb, sheet_name_rost, cols = 10, widths = 38)  # accessibility_flags (generous space for merged list)
...
setColWidths(wb, sheet_name_rost, cols = 14:15, widths = 22) # manual text timestamps
setColWidths(wb, sheet_name_rost, cols = 16:18, widths = 22) # manual call logging blanks
setColWidths(wb, sheet_name_rost, cols = 19, widths = 30)  # call_notes
