...
mapping_dict <- data.frame(
  ...
  Description = c(
    ...
    "Phone call timestamp.", "Patient intent.", "Coordinator notes.", "Appointment outcome."
  ),
  Data_Source = rep("System/Log", 24)
)
...
# .........................................................................
# SHEET 2: WEEKLY OUTREACH ROSTER (24 columns total)
# .........................................................................
...
addStyle(wb, sheet_name_rost, style_header, rows = 8, cols = 1:24)

# Explicitly pull all 24 columns in exact matching order
...
addStyle(wb, sheet_name_rost, style_text_left, rows = start_row:end_row, cols = c(1, 2, 3, 4, 5, 6, 8, 9, 11, 13, 14, 15, 16, 17, 18, 19, 21, 23, 24), gridExpand = TRUE)
...
# Conditional formatting inputs (Columns 18 to 24 for manual logs)
if (length(intervention_rows) > 0) {
  addStyle(wb, sheet_name_rost, style_active_input, rows = intervention_rows, cols = 18:24, gridExpand = TRUE)
}

if (length(control_rows) > 0) {
  addStyle(wb, sheet_name_rost, style_active_input, rows = control_rows, cols = c(18, 19, 24), gridExpand = TRUE)
  addStyle(wb, sheet_name_rost, style_control_row, rows = control_rows, cols = 20:23, gridExpand = TRUE)
}

if (length(not_in_trial_rows) > 0) {
  addStyle(wb, sheet_name_rost, style_control_row, rows = not_in_trial_rows, cols = 18:24, gridExpand = TRUE)
}

# Corrected Data Validation Dropdowns mapped to exact column numbers (20 = tier3_phone_attempt, 22 = patient_intent, 24 = appointment_outcome)
dataValidation(wb, sheet_name_rost, col = 20, rows = start_row:end_row, type = "list", value = '\"yes,no\"')
dataValidation(wb, sheet_name_rost, col = 22, rows = start_row:end_row, type = "list", value = '\"confirm,cancel,reschedule,no_response\"')
dataValidation(wb, sheet_name_rost, col = 24, rows = start_row:end_row, type = "list", value = '\"Attended,DNA,Cancelled,Rescheduled\"')
...
setColWidths(wb, sheet_name_rost, cols = 20:22, widths = 22) # manual call logging blanks
setColWidths(wb, sheet_name_rost, cols = 23, widths = 30)  # call_notes
setColWidths(wb, sheet_name_rost, cols = 24, widths = 22)  # appointment_outcome
