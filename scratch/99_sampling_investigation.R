...
dataset <- dataset %>%
 ...
 fits <- tibble(recipe = list(down_sampling = dna_recipe, no_sampling = dna_recipe_ns)) %>%
   mutate(workflow = map(recipe, \(x) workflow() %>% add_recipe(x) %>% add_model(rf_spec))) %>%
   mutate(cv_tune = map(workflow, \(x)
       fit_resamples(x, resamples = dna_folds,
